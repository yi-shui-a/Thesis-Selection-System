...
import {GetAllPaperAdmin} from "@/api/admin_api";
...
export default {
  ...
  data () {
    ...
    return {
      ...
      columns: [
        ...
        // { prop: 'selectStatus', label: '选题状态', minWidth: '100', noShowColumn: true },
        { prop: 'teacherName', label: '老师姓名', minWidth: '100', noShowColumn: true },
        { prop: 'schoolyear', label: '学年', minWidth: '100', noShowColumn: true },
        { prop: 'checkStatus', label: '审核状态', minWidth: '100', noShowColumn: true },
  ...
  methods: {
    ...
    conditionEnter(data) {
      ...
      GetAllPaperAdmin(data).then(res => {
    ...
    handleEditTable (val, type) {
      console.log('编辑', type)
      this.$router.push({path: '/admin/edit_paper', query: {pno: type.pno}})
  ...
  computed: {
    ...
    // 查询条件所需参数
    getQueryData() {
      const { title, type, teacherName, schoolyear,checkStatus } = this.queryData
      return {
        title,
        type,
        teacherName,
  ...
  created() {
    GetAllPaperAdmin(this.getQueryData).then(res => {
      this.table.data = res.data.data
